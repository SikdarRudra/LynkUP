...
function Post({ post }) {
  ...
  const handleLike = async () => {
    try {
      const result = await axiosInstance.get(`/post/like/${post._id}`);
  ...
  const handleComment = async () => {
    try {
      const result = await axiosInstance.post(`/post/comment/${post._id}`, {
        message,
      });
      const updatedPost = result.data;
  ...
  const handleSaved = async () => {
    try {
      const result = await axiosInstance.get(`/post/saved/${post._id}`);
